...
def check_admin_auth(request):
    """
    Secure implementation of admin authentication using HTTP headers
    ...
    # For backward compatibility, also check URL param
    url_token = request.args.get('admin_key', '')
    
    # Get expected admin key
    expected_admin_key = os.environ.get('ADMIN_KEY', '')
    
    # Check if either token matches
    if (token and token == expected_admin_key) or (url_token and url_token == expected_admin_key):
        return True, None
    
    # Not authorized
    return False, "Unauthorized. Valid admin key required."
...
@app.route("/admin")
def admin_panel():
    """Admin panel that always shows the login form first"""
    # Always render the login form - authentication happens client-side
    return render_template("admin.html")

@app.route("/admin/auth", methods=["POST"])
def admin_auth():
    """Dedicated endpoint for admin authentication"""
    try:
        # Get admin key from JSON body
        data = request.get_json()
        ...
        logger.error(f"Error in admin auth endpoint: {str(e)}")
        return jsonify({"error": "Authentication error"}), 500

def verify_admin_auth(request):
    """
    Verify admin authentication using Authorization header only
...
def admin_status():
    """Admin status endpoint with secure auth using headers only"""
    try:
        # Check authorization
        is_authorized, error_message = verify_admin_auth(request)
...
def metrics_endpoint():
    """Expose Prometheus metrics with secure auth"""
    try:
        # Check authorization
        is_authorized, error_message = verify_admin_auth(request)
        if not is_authorized:
...
"""
Fixed logs endpoint including all required handler functions
"""

@app.route('/logs')
def logs_endpoint():
    """Enhanced logs endpoint with secure auth"""
    try:
        # Check if logs endpoint is enabled
        if not ENABLE_LOGS_ENDPOINT:
            return jsonify({"error": "Logs endpoint is disabled"}), 404
        
        # Check authorization - support both header and URL param for backward compatibility
        ...
        auth_header = request.headers.get('Authorization', '')
        if auth_header.startswith('Bearer '):
            auth_key = auth_header[7:]  # Remove "Bearer " prefix
        else:
            auth_key = ''
            
        is_authorized = (admin_key and admin_key == expected_admin_key) or (auth_key and auth_key == expected_admin_key)
        
        if not is_authorized:
            logger.warning(f"Unauthorized logs access attempt from {request.remote_addr}")
            return jsonify({"error": "Unauthorized. Valid admin key required."}), 403
...
# Keep the original handler functions - these process different types of logs requests
